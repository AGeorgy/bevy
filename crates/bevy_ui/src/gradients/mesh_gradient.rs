...
/// A color space supported by mesh-gradient cubic interpolation.
///
...
pub enum MeshGradientColorSpace {
    /// Interpolate in `OKLab`. This is the default.
    #[default]
    Oklaba,
    /// Interpolate in sRGB.
    Srgba,
...
pub enum MeshGradientError {
    ...
    /// A patch has collapsed to an area with no usable two-dimensional extent.
    ...
    /// The conservative continuous-surface proof could not establish that a
    /// patch participates in a globally non-folding surface.
...
///
/// `MeshGradient` stores only values that pass its dimensional, numeric, and
...
/// can reject otherwise valid rotations or strongly skewed grids.
///
...
impl MeshGradient {
    /// Creates a checked mesh gradient using `OKLab` interpolation.
    ///
    ...
    pub fn new_in_color_space(
        ...
        Self::validate(width, height, &points, color_space)?;
        Ok(Self {
            width,
            height,
            points,
    ...
    pub fn try_set_point(
        ...
        self.try_edit_points(|points| {
            let Some(current) = points.get_mut(index) else {
    ...
    pub fn try_replace_points(
        ...
        let candidate =
            Self::new_in_color_space(self.width, self.height, points, self.color_space)?;
        *self = candidate;
        Ok(())
    }
    ...
    pub fn try_replace_grid(
        ...
        let candidate = Self::new_in_color_space(width, height, points, self.color_space)?;
        *self = candidate;
        Ok(())
    }
    ...
    pub fn try_set_color_space(
        ...
        let candidate =
            Self::new_in_color_space(self.width, self.height, self.points.clone(), color_space)?;
        *self = candidate;
        Ok(())
    }
...
impl<'de> serde::Deserialize<'de> for MeshGradient {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        Self::new_in_color_space(raw.width, raw.height, raw.points, raw.color_space)
...
mod tests {
    ...
    fn validation_performance_budgets() {
        use std::time::{Duration, Instant};

        fn p95(width: usize, height: usize) -> Duration {
            let points = regular_grid(width, height);
        ...
        let small = p95(4, 4);
        let maximum = p95(16, 16);
        eprintln!("mesh-gradient validation p95: 4x4={small:?}, 16x16={maximum:?}");
        assert!(small <= Duration::from_micros(100));
        assert!(maximum <= Duration::from_millis(1));
    }
}
